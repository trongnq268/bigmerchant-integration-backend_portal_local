...
def check_response_detail(responseBody, responseFieldExpect, valueExpect):
    ...
    config_file_path = os.path.join(upperDir, "MA/ma-service-international/json")
    ...
    if body != dataExpect:
        flgCheck = False
        trans_id_err = dataExpect["transaction_id"] if "transaction_id" in dataExpect else dataExpect["tranaction_id"]
        trans_type_err = dataExpect["transaction_type"] if "transaction_type" in dataExpect else dataExpect["trans_type"]
