...
def check_response_content(responseBody, responseFieldExpect, valueExpect):
    ...
    for key in responseFieldExpect:
        for tran in transactions:
            if (key not in tran):
                flgCheck = False
                listObjectFail.append(tran["merchantId"] + "-" + key + "not exits")

    # ================ Check Response Values Valid ==================#
    listCheck = list((valueExpect))
    for val in listCheck:
        if val["field"] != "" and val["field"] is not None:
            for tran in transactions:
                if val["condition"] == "equal" and val["value"] != tran[val["field"]]:
                    flgCheck = False
                    listObjectFail.append(tran["merchantId"] + "-"+ val["field"] + "-" + tran[val["field"]])
                    totalFailed += 1
                else:
                    totalPassed +=1
...
def check_response_content_ss(responseBody, responseFieldExpect, valueExpect):
    flgCheck = True
    listObjectFail = []
    if is_json(responseBody) != True:
        flgCheck = False
    ...
    # ================ Check Response Valid ==================#
    if ("total" not in body["response"]["data"]):
        flgCheck = False
    transactions = body["response"]["data"]["list"]

    for val in valueExpect:
        if val != "" and val is not None:
            for tran in transactions:
                if valueExpect["transaction_type"] != tran["transType"]["value"]:
                    flgCheck = False
                    listObjectFail.append(
                        tran["id"] + "-" + tran[val["field"]] + "-" + tran[val["field"]])
...
def compare_array_json(responseSearch, responseCheck):
    ...
    bodyCheck = responseCheck[0]
    # ================ Check Response Fields Valid ==================#
    for historyObj in bodySearch["list"]:
...
def compare_array_json_ss(responseSearch, responseCheck):
    flgCheck = True
    listObjectFail = []
    if is_json(responseSearch) != True:
        flgCheck = False
...
def check_response_search_and_detail(responseSearch, responseDetail):
    flgCheck = True
    listObjectFail = []
    if is_json(responseSearch) != True:
        flgCheck = False
    ...
    # ================ Check Response Valid ==================#
    if ("totalItems" not in bodySearch):
        flgCheck = False
    transactions = bodySearch["list"]
    # ================ Check Response Fields Valid ==================#

    for tranSearch in transactions:
        for keySearch in tranSearch.keys():
            if tranSearch[keySearch] != responseDetail[keySearch]:
                flgCheck = False
                listObjectFail.append(
                    tran["id"] + "-" + tran[val["field"]] + "-" + tran[val["field"]])
