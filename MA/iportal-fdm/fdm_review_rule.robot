...
${FDM_RULE_SEARCH}              /json/FDM_RULE_SEARCH.json
${FDM_RULE_DETAIL}              /json/FDM_RULE_DETAIL.json
${FDM_RULE_UPDATE}              /json/FDM_RULE_UPDATE.json
${FDM_RULE_DELETE}              /json/FDM_RULE_DELETE.json

*** Test Cases ***
FDM - Review Rule Search
    [Documentation]  Rule Search 
    ${request}    ${resultExpect}=    FDMRuleHandler.Make Param Json   ${CURDIR}${FDM_RULE_SEARCH}
...
FDM - Review Rule Detail
    ...
    ${request}    ${resultExpect}=    FDMRuleHandler.Make Param Json    ${CURDIR}${FDM_RULE_DETAIL}
    &{header}=    Create Dictionary    Cookie=${COOKIE}
    ...    Content-Type=application/json
    ...    X-USER-ID=xxx
    ...    User-Agent=Robot-Auto-Test
...
FDM - Review Rule Update
    ...
    ${request}    ${resultExpect}=    FDMRuleHandler.Make Param Json    ${CURDIR}${FDM_RULE_UPDATE}
    &{header}=    Create Dictionary    Cookie=${COOKIE}
    ...    Content-Type=application/json
    ...    X-USER-ID=xxx
    ...    User-Agent=Robot-Auto-Test
 ...
 FDM - Review Rule Delete
    [Documentation]    Rule Detail
    ${request}    ${resultExpect}=    FDMRuleHandler.Make Param Json    ${CURDIR}${FDM_RULE_DELETE}
    &{header}=    Create Dictionary    Cookie=${COOKIE}
    ...    Content-Type=application/json
    ...    X-USER-ID=xxx
    ...    User-Agent=Robot-Auto-Test
    ...
    FOR    ${req}    ${ret}    IN ZIP    ${request}    ${resultExpect}
        ...
        ${responseDetail}=    POST    ${API_DETAIL_URL}${req["id"]}    headers=${header}
