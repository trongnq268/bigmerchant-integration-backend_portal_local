...
def check_response_content(statusCode, resText, resExpect):
    ...
    if("reports" not in body
       or "total_refund_count" not in body
       or "total_transaction_count" not in body
       or "total_transaction_total" not in body
       or "total_refund_total" not in body):
        flgCheck = False
        ...
        if val["field"] != "" and val["field"] is not None:
            for tran in reports:
                ...
                if val["condition"] == "equal" and val["value"] != jsonTran[val["field"]]:
                    flgCheck = False
                    listObjectFail.append(jsonTran["merchant_id"] + "-"+ val["field"] + "-" + jsonTran[val["field"]])
                    totalFailed += 1
