...
def check_response_report(responseBody, responseFieldExpect, valueExpect):
    ...
    for key in responseFieldExpect:
        for tran in transactions:
            if(key not in tran):
                flgCheck = False
                listObjectFail.append(tran["reportDate"] + " - " +  tran["merchantId"] + " - " + tran["appName"] + " - "+ key + " not exits")
    ...
    for val in listCheck:
        if val["field"] != "" and val["field"] is not None:
            for tran in transactions:
                if val["condition"] == "equal" and val["value"] != tran[val["field"]]:
                    flgCheck = False
                    listObjectFail.append(tran["reportDate"] + " - " +  tran["merchantId"] + " - " + tran["appName"] + " - " + val["field"] + " - " + tran[val["field"]])
                    totalFailed += 1
                else:
                    totalPassed +=1
